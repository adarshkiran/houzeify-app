      ...
      <div className="flex flex-1 min-h-0 relative z-10">
        ...
        <div className="flex flex-col flex-1 min-h-0">
          <header className="hidden md:flex h-[64px] shrink-0 items-center justify-between px-6 lg:px-10 bg-[var(--hz-surface)] border-b border-[var(--hz-border)]">
            ...
            <button onClick={() => onNavigate('renovate-requirements')} className="text-[13px] text-[var(--hz-ink-muted)] hover:text-[var(--hz-ink)] transition-colors cursor-pointer border-0 bg-transparent" style={{ fontFamily: FONT_BODY }}>
              Back
            </button>
          </header>

          <div className="hidden md:block h-[2px] bg-[var(--hz-surface-muted)] w-full shrink-0">
            <div className="h-full bg-[var(--hz-primary)] transition-all duration-500" style={{ width: `${(4 / TOTAL_STEPS) * 100}%` }} />
          </div>

          <main className="flex-1 overflow-y-auto" style={{ scrollbarWidth: 'none' }}>
            <div className="max-w-[900px] mx-auto px-5 sm:px-8 lg:px-10 pt-8 pb-12 flex flex-col gap-8">
              <div className="flex flex-col gap-2">
                <span className="text-[12px] tracking-[0.06em] uppercase text-[var(--hz-ink-subtle)]" style={{ fontFamily: FONT_MONO }}>Step 4 of {TOTAL_STEPS}</span>
                <h2 className="text-[24px] sm:text-[28px] font-semibold text-[var(--hz-ink)] m-0" style={{ fontFamily: FONT_HEAD }}>Set your renovation budget</h2>
              </div>
                ...
                <div className="flex flex-col gap-3">
                  ...
                  {budget === 'custom' && (
                    <div className="flex items-center gap-2 mt-1">
                      <span className="text-[14px] text-[var(--hz-ink-muted)]" style={{ fontFamily: FONT_BODY }}>₹</span>
                      <input
                        type="text"
                        inputMode="numeric"
                        value={customBudget}
                        ...
                        className="h-11 px-4 rounded-[10px] border border-[var(--hz-border)] text-[14px] text-[var(--hz-ink)] outline-none focus:border-[var(--hz-primary)] transition-colors w-[200px]"
