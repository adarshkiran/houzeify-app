  ...
  return (
    <div className="flex md:hidden h-14 items-center justify-between px-4 bg-[var(--hz-surface)] border-b border-[var(--hz-border)] shrink-0 z-10">
      <div className="flex items-center gap-2">
        ...
        <span className="text-[16px] font-semibold text-[var(--hz-ink)]" style={{ fontFamily: FONT_HEAD }}>Hair Studio</span>
      </div>
  ...
  return (
    <header className="hidden md:flex h-[64px] shrink-0 items-center justify-between px-6 lg:px-8 bg-[var(--hz-surface)] border-b border-[var(--hz-border)]">
      <div className="flex items-center gap-3">
        ...
        <h1 className="text-[20px] font-semibold text-[var(--hz-ink)] m-0" style={{ fontFamily: FONT_HEAD }}>Hair Studio for Women</h1>
      </div>
      <div className="flex items-center gap-2">
  ...
  return (
    ...
    >
      <div className="relative flex flex-col gap-4 px-5 sm:px-10 lg:px-12 py-8 sm:py-10">
        <span className="text-[11px] tracking-[0.12em] uppercase text-[var(--hz-primary)] font-semibold" style={{ fontFamily: FONT_MONO }}>Hair Studio</span>
    ...
    <div className="flex flex-col sm:flex-row items-stretch gap-4 sm:gap-5 rounded-[12px] p-4" style={CARD_SURFACE}>
      <div className="relative shrink-0 w-full h-[100px] sm:w-[170px] sm:h-auto rounded-[8px] flex items-center justify-center" style={PHOTO_PANEL}>
        <div className="text-center leading-[1.05]">
          <span className="block text-[28px] sm:text-[38px] font-semibold text-[var(--hz-ink)]" style={{ fontFamily: FONT_HEAD }}>{discountPct}%</span>
        ...
        <div className="flex flex-row sm:flex-col items-center sm:items-end justify-between shrink-0 sm:w-[140px] gap-3">
          <div className="text-left sm:text-right">
            <div className="text-[14px] whitespace-nowrap" style={{ fontFamily: FONT_BODY }}>
              <span className="line-through text-[var(--hz-ink-subtle)]">₹{pkg.originalPrice.toLocaleString('en-IN')}</span> <span className="font-semibold text-[var(--hz-ink)]">₹{pkg.price.toLocaleString('en-IN')}</span>
            </div>
...
const PKG_ROW_DEFAULT = 'bg-[var(--hz-surface)] border-[var(--hz-border)] hover:border-[var(--hz-primary)]'
const PKG_ROW_SELECTED = 'bg-[var(--hz-primary-wash)] border-[var(--hz-primary)]'
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center sm:p-4" style={{ background: 'rgba(36,35,38,0.45)' }} onClick={onClose}>
      <div
        ...
        className="w-full sm:max-w-[560px] h-[92vh] sm:h-auto sm:max-h-[85vh] rounded-t-[20px] sm:rounded-[16px] bg-[var(--hz-surface)] flex flex-col overflow-hidden"
        style={{ boxShadow: '0 12px 40px rgba(0,0,0,0.18)' }}
        onClick={e => e.stopPropagation()}
      >
        <div className="flex flex-col gap-1.5 px-5 py-5 border-b border-[var(--hz-surface-muted)] shrink-0" style={{ background: 'linear-gradient(120deg, #FFF3EA 0%, #FDEBEF 100%)' }}>
          <div className="flex items-start justify-between gap-3">
            <h2 id="hair-pkg-customiser-title" className="text-[20px] font-semibold text-[var(--hz-ink)] leading-tight m-0" style={{ fontFamily: FONT_HEAD }}>{pkg.name}</h2>
            <button onClick={onClose} aria-label="Close package editor" className="w-8 h-8 -mr-1 -mt-1 shrink-0 flex items-center justify-center text-[var(--hz-ink-muted)] hover:bg-[var(--hz-surface)]/70 hover:text-[var(--hz-ink)] rounded-[10px] transition-all border-0 bg-transparent cursor-pointer">
              <IcoCloseX />
            </button>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-5 flex flex-col gap-6">
          {categories.map(cat => (
            <fieldset key={cat.id} className="flex flex-col gap-2 border-0 p-0 m-0">
              <legend className="text-[16px] font-semibold text-[var(--hz-ink)] px-0 mb-1" style={{ fontFamily: FONT_HEAD }}>{cat.label}</legend>
              {cat.options.map(opt => (
                <label key={opt.id} className={`${PKG_ROW_BASE} ${sel[cat.id] === opt.id ? PKG_ROW_SELECTED : PKG_ROW_DEFAULT}`}>
                  <input type="radio" name={`hair-pkg-${cat.id}`} checked={sel[cat.id] === opt.id} onChange={() => select(cat.id, opt.id)} className="w-4 h-4 shrink-0" style={{ accentColor: 'var(--hz-ink)' }} />
                  <span className="flex-1 text-[14.5px] font-medium text-[var(--hz-ink)]" style={{ fontFamily: FONT_BODY }}>{opt.label}</span>
                  <span className="text-[13px] text-[var(--hz-ink-subtle)] shrink-0" style={{ fontFamily: FONT_BODY }}>₹{opt.price.toLocaleString('en-IN')}</span>
                </label>
              ))}
              <label className={`${PKG_ROW_BASE} ${sel[cat.id] === null ? PKG_ROW_SELECTED : PKG_ROW_DEFAULT}`}>
                <input type="radio" name={`hair-pkg-${cat.id}`} checked={sel[cat.id] === null} onChange={() => select(cat.id, null)} className="w-4 h-4 shrink-0" style={{ accentColor: 'var(--hz-ink)' }} />
                <span className="flex-1 text-[14.5px] font-medium text-[var(--hz-ink)]" style={{ fontFamily: FONT_BODY }}>{cat.noneLabel}</span>
              </label>
            </fieldset>
          ))}
        </div>
        ...
        <div className="shrink-0 flex flex-col gap-0">
          ...
          <div className="border-t border-[var(--hz-surface-muted)] bg-[var(--hz-surface)] px-5 py-4 flex flex-col gap-2">
            {validationMessage && (
              <span role="alert" className="text-[12px] text-[#B3261E] font-medium" style={{ fontFamily: FONT_BODY }}>{validationMessage}</span>
            )}
            <div className="flex items-center justify-between gap-3">
              <span className="text-[19px] font-semibold text-[var(--hz-ink)] whitespace-nowrap" style={{ fontFamily: FONT_HEAD }}>
                ₹{finalPrice.toLocaleString('en-IN')}{saved > 0 && <span className="line-through text-[var(--hz-ink-subtle)] text-[14px] font-normal ml-1.5">₹{rawTotal.toLocaleString('en-IN')}</span>}
              </span>
              <button
                onClick={handleAddToCart}
                className="h-11 px-6 rounded-[10px] bg-[var(--hz-primary)] text-white text-[14px] font-semibold cursor-pointer hover:brightness-90 active:scale-[0.99] transition-all border-0 shrink-0"
                style={{ fontFamily: FONT_BODY, boxShadow: '0 2px 8px rgba(114,46,209,0.25)' }}
              >
                Add to cart
              </button>
