  ...
  return (
    <div className="flex md:hidden h-14 items-center justify-between px-4 bg-white border-b border-[#E3DDD7] shrink-0 z-10">
      <div className="flex items-center gap-2">
        ...
        <span className="text-[16px] font-semibold text-[#242326]" style={{ fontFamily: FONT_HEAD }}>Civil Work</span>
      </div>
  ...
  return (
    <header className="hidden md:flex h-[64px] shrink-0 items-center justify-between px-6 lg:px-8 bg-[#FFFFFF] border-b border-[#E3DDD7]">
      <div className="flex items-center gap-3">
        ...
        <h1 className="text-[20px] font-semibold text-[#242326] m-0" style={{ fontFamily: FONT_HEAD }}>Civil Work</h1>
      </div>
      <div className="flex items-center gap-2">
  ...
  return (
    ...
    >
      <div className="relative flex flex-col gap-4 px-5 sm:px-10 lg:px-12 py-8 sm:py-10">
        <span className="text-[11px] tracking-[0.12em] uppercase text-[#722ED1] font-semibold" style={{ fontFamily: FONT_MONO }}>Essential Home Repairs</span>
...
const CHIP_DEFAULT = 'h-8 px-3 rounded-[9px] border text-[12px] font-semibold cursor-pointer transition-all shrink-0 bg-white border-[#E3DDD7] text-[#242326] hover:border-[#722ED1] hover:text-[#722ED1]'
const CHIP_SELECTED = 'h-8 px-3 rounded-[9px] border text-[12px] font-semibold cursor-pointer transition-all shrink-0 bg-[#722ED1] border-[#722ED1] text-white'
const ADD_BTN_DEFAULT = 'h-9 px-4 rounded-[10px] border text-[12.5px] font-semibold cursor-pointer transition-all bg-white border-[#E3DDD7] text-[#722ED1] hover:bg-[#F3EAFF] hover:border-[#722ED1]'
const ADD_BTN_ADDED = 'h-9 px-4 rounded-[10px] border text-[12.5px] font-semibold cursor-pointer transition-all bg-[#F3EAFF] border-[#722ED1] text-[#722ED1]'
  ...
  return (
    <div className="flex flex-col gap-3 rounded-[12px] p-4" style={CARD_SURFACE}>
      <div className="flex items-start gap-3">
        <div className="w-14 h-14 shrink-0 rounded-[8px] overflow-hidden flex items-center justify-center bg-[#F9F5FF] text-[#722ED1]">
          {item.icon}
        </div>
        <div className="flex-1 min-w-0 flex flex-col gap-1">
          <span className="text-[15px] font-semibold text-[#242326] leading-tight" style={{ fontFamily: FONT_HEAD }}>{item.title}</span>
          <span className="text-[12.5px] text-[#68636D] leading-snug" style={{ fontFamily: FONT_BODY }}>{item.description}</span>
          {item.rating && (
            <span className="flex items-center gap-1 text-[11px] font-semibold text-[#722ED1] mt-0.5" style={{ fontFamily: FONT_HEAD }}><IcoStar size={11} /> {item.rating}</span>
          )}
        </div>
      </div>
      ...
      {(item.pricingModel === 'hourly' || item.pricingModel === 'sqft') && hasTiers && (
        <>
          <div className="flex items-baseline gap-2 flex-wrap">
            <span className="text-[14px] font-semibold text-[#242326]" style={{ fontFamily: FONT_BODY }}>{item.unitRateLabel}</span>
            {item.minLabel && <span className="text-[12px] text-[#9A949D]" style={{ fontFamily: FONT_BODY }}>{item.minLabel}</span>}
          </div>
          ...
          <div className="flex items-center justify-between gap-3">
            {assessmentSelected ? (
              <span className="text-[12.5px] text-[#68636D]" style={{ fontFamily: FONT_BODY }}>Price after site assessment</span>
            ) : selectedTier ? (
              <span className="text-[15px] font-semibold text-[#242326]" style={{ fontFamily: FONT_HEAD }}>₹{selectedTier.price.toLocaleString('en-IN')}</span>
            ) : <span />}
            <button
      ...
      {item.pricingModel === 'per-job' && (
        <div className="flex items-center justify-between gap-3 flex-wrap">
          <div className="flex flex-col gap-0.5">
            <span className="text-[15px] font-semibold text-[#242326]" style={{ fontFamily: FONT_HEAD }}>From ₹{item.flatPrice!.toLocaleString('en-IN')} / job</span>
            {item.noteLabel && <span className="text-[12px] text-[#9A949D]" style={{ fontFamily: FONT_BODY }}>{item.noteLabel}</span>}
          </div>
          <button
      ...
      {item.pricingModel === 'site-assessment' && (
        <div className="flex items-center justify-between gap-3 flex-wrap">
          <span className="text-[13px] font-semibold text-[#68636D]" style={{ fontFamily: FONT_BODY }}>Assessment required</span>
          <button
  ...
  return (
    <div className="bg-white border border-[#E3DDD7] rounded-[16px] p-5 flex flex-col gap-4" style={{ boxShadow: '0 1px 8px rgba(0,0,0,0.04)' }}>
      ...
      <div className="flex flex-col">
        {cart.map((c, i) => (
          <div key={c.cartId} className={`flex flex-col gap-2 py-3 ${i > 0 ? 'border-t border-[#F4F0EC]' : 'pt-0'}`}>
            ...
            <span className="text-[13px] font-semibold text-[#242326]" style={{ fontFamily: FONT_BODY }}>{c.price === 0 ? 'Price after assessment' : `₹${c.price.toLocaleString('en-IN')}`}</span>
            <button
      ...
      <div className="flex items-center justify-between gap-3 pt-3 border-t border-[#F4F0EC]">
        <span className="text-[17px] font-semibold text-[#242326]" style={{ fontFamily: FONT_HEAD }}>₹{total.toLocaleString('en-IN')}</span>
        <button
          onClick={onViewCart}
          ...
          <main className="flex-1 overflow-y-auto" style={{ padding: '28px 24px', scrollbarWidth: 'none' }}>
            <div className="flex flex-col gap-6" style={{ maxWidth: 1200, margin: '0 auto' }}>
              ...
              <div className="flex flex-col gap-1.5">
                <span className="text-[12px] tracking-[0.10em] uppercase text-[#722ED1] font-semibold" style={{ fontFamily: FONT_MONO }}>Essential Home Repairs</span>
