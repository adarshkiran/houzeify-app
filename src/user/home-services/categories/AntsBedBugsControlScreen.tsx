// ─── Ants & Bed Bugs Control — Cleaning & Pest Control → Pest Control →
// Ants & Bed Bugs Control ────────────────────────────────────────────
//
// Built from a real-page reference PDF the user supplied (UB-BB18) —
// used for its "Select a service" catalogue (2 sections: Bed Bugs
// Control, Ant Control) and every service's own rating/reviews/price/
// description, taken as given.
//
// Each of the 7 items' own real per-tier options ("1 bhk" … "5 bhk",
// "2000-3000 sq. ft." … "5000+ sq. ft.", "Only kitchen" … "5 bathrooms
// & kitchen") were later supplied via reference screenshots and are
// wired in as a real OptionsModal picker — same component and pattern
// BathroomCleaningScreen/CockroachControlScreen/TermiteControlScreen's
// own options-bearing rows use — rather than the plain "N options"
// hint this page used before that data existed. A handful of the
// larger, less-booked tiers show a real duration (e.g. "2 hrs 55
// mins") instead of a rating in the reference — kept as a duration
// field on the option, shown with a clock icon in place of the star,
// rather than inventing a rating that wasn't there.
//
...
// its own real-brand promotional video banner ("Unique 2 visit
// treatment"), footer, brand promise card, and whatever was in its own
// cart at capture time — this page's own HeroBanner/CategoryBanners
// use original Houzeify copy instead, same as every sibling screen.
//
...
const IcoClock = ({ size = 11 }: { size?: number }) => (
...
const IcoBed = () => (
  <svg width="22" height="22" viewBox="0 0 20 20" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
    <path d="M2 15.5V7a1 1 0 011-1h14a1 1 0 011 1v8.5" />
    <path d="M2 12.5h16M4 8.5v4M8 8.5a2 2 0 114 0v0" />
    <path d="M2 15.5v2M18 15.5v2" />
  </svg>
)
const IcoAnt = () => (
  <svg width="22" height="22" viewBox="0 0 20 20" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
    ...
    <circle cx="10" cy="14.2" r="2.4" />
    <path d="M6.5 8.5L3 6.5M13.5 8.5L17 6.5M6 10.5H2M14 10.5h4M6.2 13.5L2.5 15.5M13.8 13.5l3.7 2" />
  </svg>
)
...
interface AntsBedBugsItem {
  id: string
  title: string
  section: 'bed-bugs-control' | 'ant-control'
  rating: string
  price: number
  priceLabel?: 'starts-at'
  description?: string
  note?: string
  options?: AntsBedBugsItemOption[]
  icon: React.ReactNode
}
// A real, concrete sub-option — e.g. "1 bhk" … "5 bhk", "2000-3000 sq.
// ft." … "5000+ sq. ft.", or "Only kitchen" … "5 bathrooms & kitchen" —
// each with its own real price, taken from a reference screenshot the
// user supplied. A handful of the larger, less-booked tiers show a
// real duration instead of a rating in the reference (not enough
// bookings yet for a review count) — kept as `duration`, never a
// fabricated rating.
interface AntsBedBugsItemOption {
  id: string
  label: string
  price: number
  rating?: string
...
const ANTS_BEDBUGS_ITEMS: AntsBedBugsItem[] = [
  // ── Bed Bugs Control ──
  {
    id: 'bb-bed-bugs-control', title: 'Bed bugs control', section: 'bed-bugs-control', rating: '4.77 (39K reviews)', price: 1899, priceLabel: 'starts-at', description: 'Essential pre-service inspection of the entire home', note: 'Unique 2-visit treatment to target eggs, nymphs & adult pests', icon: <IcoBed />,
    options: [
      { id: 'bb-bed-1bhk', label: '1 bhk', price: 1899, rating: '4.78 (21K reviews)' },
      { id: 'bb-bed-2bhk', label: '2 bhk', price: 2197, rating: '4.77 (14K reviews)' },
      { id: 'bb-bed-3bhk', label: '3 bhk', price: 2799, rating: '4.74 (4K reviews)' },
      { id: 'bb-bed-4bhk', label: '4 bhk', price: 3397, rating: '4.71 (374 reviews)' },
      { id: 'bb-bed-5bhk', label: '5 bhk', price: 3899, rating: '4.64 (81 reviews)' },
    ],
  },

  // ── Ant Control ──
  {
    id: 'bb-apt-ant-with-utensil', title: 'Apartment ant control (with utensil removal)', section: 'ant-control', rating: '4.80 (1K reviews)', price: 1849, priceLabel: 'starts-at', description: 'Complete ant treatment for confined spaces', note: 'Includes thorough inspection, chemical spray & hole sealing', icon: <IcoApartmentBungalow />,
    options: [
      { id: 'bb-apt-wu-1bhk', label: '1 bhk', price: 1849, rating: '4.88 (152 reviews)' },
      { id: 'bb-apt-wu-2bhk', label: '2 bhk', price: 1899, rating: '4.84 (410 reviews)' },
      { id: 'bb-apt-wu-3bhk', label: '3 bhk', price: 1999, rating: '4.76 (414 reviews)' },
      { id: 'bb-apt-wu-4bhk', label: '4 bhk', price: 2199, rating: '4.64 (93 reviews)' },
      { id: 'bb-apt-wu-5bhk', label: '5 bhk', price: 2199, rating: '4.97 (19 reviews)' },
    ],
  },
  {
    id: 'bb-apt-ant-without-utensil', title: 'Apartment ant control (without utensil removal)', section: 'ant-control', rating: '4.82 (1K reviews)', price: 1549, priceLabel: 'starts-at', description: 'Complete ant treatment for confined spaces', note: 'Includes thorough inspection, chemical spray & hole sealing', icon: <IcoApartmentBungalow />,
    options: [
      { id: 'bb-apt-nu-1bhk', label: '1 bhk', price: 1549, rating: '4.85 (234 reviews)' },
      { id: 'bb-apt-nu-2bhk', label: '2 bhk', price: 1647, rating: '4.82 (404 reviews)' },
      { id: 'bb-apt-nu-3bhk', label: '3 bhk', price: 1747, rating: '4.81 (459 reviews)' },
      { id: 'bb-apt-nu-4bhk', label: '4 bhk', price: 1949, rating: '4.69 (76 reviews)' },
      { id: 'bb-apt-nu-5bhk', label: '5 bhk', price: 2199, rating: '4.94 (17 reviews)' },
    ],
  },
  {
    id: 'bb-bungalow-ant-with-utensil', title: 'Bungalow ant control (with utensil removal)', section: 'ant-control', rating: '4.72 (48 reviews)', price: 2398, priceLabel: 'starts-at', description: 'Extensive ant protection for larger areas', icon: <IcoApartmentBungalow />,
    options: [
      { id: 'bb-bng-wu-2000-3000', label: '2000-3000 sq. ft.', price: 2398, rating: '4.72 (48 reviews)' },
      { id: 'bb-bng-wu-3000-4000', label: '3000-4000 sq. ft.', price: 2598, duration: '2 hrs 55 mins' },
      { id: 'bb-bng-wu-4000-5000', label: '4000-5000 sq. ft.', price: 2898, duration: '3 hrs 10 mins' },
      { id: 'bb-bng-wu-5000-plus', label: '5000+ sq. ft.', price: 3099, duration: '3 hrs 35 mins' },
    ],
  },
  {
    id: 'bb-bungalow-ant-without-utensil', title: 'Bungalow ant control (without utensil removal)', section: 'ant-control', rating: '4.74 (61 reviews)', price: 2097, priceLabel: 'starts-at', description: 'Extensive ant protection for larger areas', icon: <IcoApartmentBungalow />,
    options: [
      { id: 'bb-bng-nu-2000-3000', label: '2000-3000 sq. ft.', price: 2097, rating: '4.74 (48 reviews)' },
      { id: 'bb-bng-nu-3000-4000', label: '3000-4000 sq. ft.', price: 2297, rating: '4.73 (13 reviews)' },
      { id: 'bb-bng-nu-4000-5000', label: '4000-5000 sq. ft.', price: 2598, duration: '2 hrs 45 mins' },
      { id: 'bb-bng-nu-5000-plus', label: '5000+ sq. ft.', price: 2799, duration: '3 hrs 5 mins' },
    ],
  },
  {
    id: 'bb-kb-ant-with-utensil', title: 'Ant control – kitchen/bathroom (with utensil removal)', section: 'ant-control', rating: '4.85 (618 reviews)', price: 1249, priceLabel: 'starts-at', description: 'Complete ant treatment for confined spaces', note: "We'll remove utensils before the service begins", icon: <IcoKitchenBath />,
    options: [
      { id: 'bb-kb-wu-only-kitchen', label: 'Only kitchen', price: 1249, rating: '4.95 (407 reviews)' },
      { id: 'bb-kb-wu-1bath-kitchen', label: '1 bathroom & kitchen', price: 1349, rating: '4.64 (121 reviews)' },
      { id: 'bb-kb-wu-2bath-kitchen', label: '2 bathrooms & kitchen', price: 1449, rating: '4.70 (64 reviews)' },
      { id: 'bb-kb-wu-3bath-kitchen', label: '3 bathrooms & kitchen', price: 1497, rating: '4.75 (26 reviews)' },
      { id: 'bb-kb-wu-4bath-kitchen', label: '4 bathrooms & kitchen', price: 1548, duration: '1 hr 55 mins' },
      { id: 'bb-kb-wu-5bath-kitchen', label: '5 bathrooms & kitchen', price: 1599, duration: '2 hrs 5 mins' },
    ],
  },
  {
    id: 'bb-kb-ant-without-utensil', title: 'Ant control – kitchen/bathroom (without utensil removal)', section: 'ant-control', rating: '4.86 (741 reviews)', price: 998, priceLabel: 'starts-at', description: 'Complete ant treatment for confined spaces', note: 'Excludes removal of utensils & objects before the service begins', icon: <IcoKitchenBath />,
    options: [
      { id: 'bb-kb-nu-only-kitchen', label: 'Only kitchen', price: 998, rating: '4.88 (432 reviews)' },
      { id: 'bb-kb-nu-1bath-kitchen', label: '1 bathroom & kitchen', price: 1098, rating: '4.84 (201 reviews)' },
      { id: 'bb-kb-nu-2bath-kitchen', label: '2 bathrooms & kitchen', price: 1198, rating: '4.78 (76 reviews)' },
      { id: 'bb-kb-nu-3bath-kitchen', label: '3 bathrooms & kitchen', price: 1247, rating: '4.98 (32 reviews)' },
      { id: 'bb-kb-nu-4bath-kitchen', label: '4 bathrooms & kitchen', price: 1297, duration: '1 hr 25 mins' },
      { id: 'bb-kb-nu-5bath-kitchen', label: '5 bathrooms & kitchen', price: 1347, duration: '1 hr 35 mins' },
    ],
  },
]

const SECTIONS: { id: AntsBedBugsItem['section']; label: string; icon: React.ReactNode }[] = [
  { id: 'bed-bugs-control', label: 'Bed Bugs Control', icon: <IcoBed /> },
  { id: 'ant-control', label: 'Ant Control', icon: <IcoAnt /> },
]

// Original Houzeify copy for each section's CategoryBanner — same
// pattern as every other service-detail screen.
const SECTION_BANNERS: Record<AntsBedBugsItem['section'], { title: string; subtitle: string }> = {
  'bed-bugs-control': { title: 'Sleep easy again', subtitle: 'A unique 2-visit treatment that targets eggs, nymphs & adults.' },
  'ant-control': { title: 'Ant-proof your home', subtitle: 'Complete ant treatment sized to your apartment, bungalow or kitchen.' },
}

function getAntsBedBugsItem(id: string, section: AntsBedBugsItem['section']): AntsBedBugsItem {
  const item = ANTS_BEDBUGS_ITEMS.find(i => i.id === id && i.section === section)
  if (!item) throw new Error(`Unknown Ants & Bed Bugs Control item id: ${id} in section ${section}`)
  return item
}
/** A single lookup point for "what has options" — mirrors
 *  CockroachControlScreen/TermiteControlScreen's own getOptionsSource. */
function getOptionsSource(id: string, section: AntsBedBugsItem['section']): { title: string; rating: string; options: AntsBedBugsItemOption[]; icon: React.ReactNode } {
  const item = getAntsBedBugsItem(id, section)
  ...
  return (
    <div className="flex md:hidden h-14 items-center justify-between px-4 bg-white border-b border-[#E3DDD7] shrink-0 z-10">
      <div className="flex items-center gap-2">
        ...
        <span className="text-[16px] font-semibold text-[#242326]" style={{ fontFamily: FONT_HEAD }}>Ants &amp; Bed Bugs Control</span>
      </div>
  ...
  return (
    <header className="hidden md:flex h-[64px] shrink-0 items-center justify-between px-6 lg:px-8 bg-[#FFFFFF] border-b border-[#E3DDD7]">
      <div className="flex items-center gap-3">
        ...
        <h1 className="text-[20px] font-semibold text-[#242326] m-0" style={{ fontFamily: FONT_HEAD }}>Ants &amp; Bed Bugs Control</h1>
      </div>
      <div className="flex items-center gap-2">
        ...
        <button
          onClick={() => onNavigate('booking-details', { hoziehelper_checkout_origin: 'ants-bedbugs-control' })}
  ...
  return (
    ...
    >
      <div className="relative flex flex-col gap-4 px-5 sm:px-10 lg:px-12 py-8 sm:py-10">
        <span className="text-[11px] tracking-[0.12em] uppercase text-[#722ED1] font-semibold" style={{ fontFamily: FONT_MONO }}>Ants &amp; Bed Bugs Control</span>
      ...
      <div className="absolute right-0 bottom-0 top-0 w-[34%] sm:w-[38%] flex items-center justify-center text-[#722ED1] opacity-20">
        <div style={{ transform: 'scale(3.2)' }}><IcoAnt /></div>
      </div>
    </div>
  )
}
...
function AntsBedBugsLineItemRow({ item, cartCount, getCartCount, onSelect, onViewDetails }: {
  item: AntsBedBugsItem
  cartCount: number
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: 'rgba(36,35,38,0.45)' }} onClick={onClose}>
      ...
      >
        ...
        <div className="flex flex-col gap-3 p-5">
          ...
          {item.options && (
            <span className="text-[12px] text-[#9A949D]" style={{ fontFamily: FONT_BODY }}>{item.options.length} options available</span>
          )}
          <button
            onClick={onSelect}
...
// user-supplied option data (e.g. "1 bhk" … "5 bhk"). Same calm card
// language as the rest of this file, same component every other
// options-bearing page in this app uses — not a separate visual
// system. An option without a real rating shows its real duration with
// a clock icon instead (never a fabricated rating). ──────────────────

function OptionsModal({ source, cartCountForOption, onAddOption, onClose }: {
  source: { id: string; section: AntsBedBugsItem['section'] }
  cartCountForOption: (optionId: string) => number
  onAddOption: (option: AntsBedBugsItemOption) => void
  onClose: () => void
}) {
              ...
              <div key={opt.id} className="flex items-center gap-3 rounded-[12px] p-3" style={CARD_SURFACE}>
                ...
                <div className="flex-1 min-w-0 flex flex-col gap-1">
                  ...
                  {opt.rating ? (
                    ...
                    <span className="flex items-center gap-1 text-[11px] font-semibold text-[#68636D]" style={{ fontFamily: FONT_HEAD }}><IcoClock /> {opt.duration}</span>
                  ) : null}
...
export default function AntsBedBugsControlScreen({
  onNavigate,
}: {
  ...
  const [detailItem, setDetailItem] = useState<{ id: string; section: AntsBedBugsItem['section'] } | null>(null)
  const [expandedItem, setExpandedItem] = useState<{ id: string; section: AntsBedBugsItem['section'] } | null>(null)

  const addToCart = (entry: Addable) => {
    addItem({
      cartId: entry.id,
      serviceEntry: 'home-services',
      categoryId: 'ants-bedbugs-control',
      serviceId: entry.id,
  ...
  const addOptionToCart = (source: { id: string; section: AntsBedBugsItem['section'] }, option: AntsBedBugsItemOption) => {
  ...
  const jumpToSection = (id: string) => {
    document.getElementById(`antsbedbugs-section-${id}`)?.scrollIntoView({ behavior: 'smooth', block: 'start' })
  }
  ...
  const viewCart = () => {
    onNavigate('booking-details', {
      hoziehelper_checkout_origin: 'ants-bedbugs-control',
    })
  }
  ...
  return (
    <div className="flex flex-col relative" style={{ height: '100%', backgroundColor: '#FFFFFF' }}>
            ...
            <div className="flex flex-col gap-6" style={{ maxWidth: 1200, margin: '0 auto' }}>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-[1fr_300px] gap-6 items-start">
                ...
                <div className="flex flex-col gap-6 min-w-0">
                  <HeroBanner onExplore={() => jumpToSection('bed-bugs-control')} />
                  <ServiceTabs onJump={jumpToSection} />

                  {SECTIONS.map(section => {
                    const items = ANTS_BEDBUGS_ITEMS.filter(i => i.section === section.id)
                    return (
                      <div key={section.id} id={`antsbedbugs-section-${section.id}`} className="flex flex-col gap-3" style={{ scrollMarginTop: 230 }}>
      ...
      {detailItem && (
        <ServiceDetailModal
          item={getAntsBedBugsItem(detailItem.id, detailItem.section)}
          cartCount={cartCountForItem(detailItem.id)}
          onSelect={() => {
            const item = getAntsBedBugsItem(detailItem.id, detailItem.section)
            if (item.options) {
