  ...
  return (
    <div className="flex md:hidden h-14 items-center justify-between px-4 bg-[var(--hz-surface)] border-b border-[var(--hz-border)] shrink-0 z-10">
      <div className="flex items-center gap-2">
        ...
        <span className="text-[16px] font-semibold text-[var(--hz-ink)]" style={{ fontFamily: FONT_HEAD }}>Walls &amp; Rooms Painting</span>
      </div>
  ...
  return (
    <header className="hidden md:flex h-[64px] shrink-0 items-center justify-between px-6 lg:px-8 bg-[var(--hz-surface)] border-b border-[var(--hz-border)]">
      <div className="flex items-center gap-3">
        ...
        <h1 className="text-[20px] font-semibold text-[var(--hz-ink)] m-0" style={{ fontFamily: FONT_HEAD }}>Walls &amp; Rooms Painting</h1>
      </div>
      <div className="flex items-center gap-2">
...
function OptionTile({ label, price, note, selected, onSelect }: { label: string; price: number; note?: string; selected: boolean; onSelect: () => void }) {
  return (
    ...
    >
      ...
      <span className="text-[13px] text-[var(--hz-ink)]" style={{ fontFamily: FONT_BODY }}>{price === 0 ? '₹0' : `₹${price.toLocaleString('en-IN')}`}</span>
    </button>
  )
}
...
function WallPaintingEstimateModal({ itemId, initialCartCount, onAddToCart, onClose }: {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: 'rgba(36,35,38,0.45)' }} onClick={onClose}>
      <div
        className="w-full max-w-[560px] max-h-[85vh] rounded-[16px] bg-[var(--hz-surface)] flex flex-col relative"
        style={{ boxShadow: '0 12px 40px rgba(0,0,0,0.18)' }}
        onClick={e => e.stopPropagation()}
      >
        <div className="flex items-start justify-between gap-3 px-5 py-4 border-b border-[var(--hz-surface-muted)] sticky top-0 bg-[var(--hz-surface)] z-10">
          <div className="flex flex-col gap-0.5 min-w-0">
            <span className="text-[18px] font-semibold text-[var(--hz-ink)] leading-tight" style={{ fontFamily: FONT_HEAD }}>{config.title}</span>
            <span className="text-[12.5px] text-[var(--hz-ink-muted)]" style={{ fontFamily: FONT_BODY }}>{config.startsAtNote}</span>
          </div>
          <button onClick={onClose} aria-label="Close" className="w-8 h-8 -mr-1 -mt-1 shrink-0 flex items-center justify-center text-[var(--hz-ink-muted)] hover:bg-[var(--hz-surface-muted)] hover:text-[var(--hz-ink)] rounded-[10px] transition-all border-0 bg-transparent cursor-pointer">
            <IcoCloseX />
          </button>
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4 flex flex-col gap-5" style={{ scrollbarWidth: 'none' }}>
          {config.wallCountOptions && (
            <div className="flex flex-col gap-2">
              <span className="text-[14px] font-semibold text-[var(--hz-ink)]" style={{ fontFamily: FONT_HEAD }}>{config.wallCountLabel ?? 'Select number of walls'}</span>
          ...
          {config.paintTypeOptions && (
            <div className="flex flex-col gap-2">
              <span className="text-[14px] font-semibold text-[var(--hz-ink)]" style={{ fontFamily: FONT_HEAD }}>Select paint type</span>
              <div className="flex gap-2.5 overflow-x-auto pb-1" style={{ scrollbarWidth: 'none' }}>
          ...
          {config.damageOptions && (
            <div className="flex flex-col gap-2">
              <span className="text-[14px] font-semibold text-[var(--hz-ink)]" style={{ fontFamily: FONT_HEAD }}>{config.damageLabel ?? 'Select any damage/seepage issue'}</span>
              <div className="flex gap-2.5 overflow-x-auto pb-1" style={{ scrollbarWidth: 'none' }}>
          ...
          {zoneOptions.length > 0 && (
            <div className="flex flex-col gap-2">
              <span className="text-[14px] font-semibold text-[var(--hz-ink)]" style={{ fontFamily: FONT_HEAD }}>{config.zoneLabel ?? 'Select area'}</span>
              <div className="flex gap-2.5 overflow-x-auto pb-1" style={{ scrollbarWidth: 'none' }}>
                {zoneOptions.map(z => (
                  <AddonTile
                    key={z.id}
          ...
          {addonOptions.length > 0 && (
            <div className="flex flex-col gap-2">
              <span className="text-[14px] font-semibold text-[var(--hz-ink)]" style={{ fontFamily: FONT_HEAD }}>Select add-ons</span>
              <div className="flex gap-2.5 overflow-x-auto pb-1" style={{ scrollbarWidth: 'none' }}>
                {addonOptions.map(a => (
                  <AddonTile
                    key={a.id}
          ...
          {discount === 0 && (
            <div className="flex items-center gap-2 rounded-[10px] px-3 py-2.5" style={{ backgroundColor: 'var(--hz-surface-muted)' }}>
              <span className="text-[12.5px] text-[var(--hz-ink-muted)]" style={{ fontFamily: FONT_BODY }}>Add ₹{(WALL_PAINTING_DISCOUNT_THRESHOLD + 1 - total).toLocaleString('en-IN')} more to get 10% off</span>
            </div>
          )}
          {discount > 0 && (
          ...
          <div className="flex flex-col gap-2 pt-2 border-t border-[var(--hz-surface-muted)]">
            <span className="text-[14px] font-semibold text-[var(--hz-ink)]" style={{ fontFamily: FONT_HEAD }}>Price includes</span>
        ...
        <div className="flex items-center justify-between gap-3 px-5 py-4 border-t border-[var(--hz-surface-muted)] sticky bottom-0 bg-[var(--hz-surface)]">
          <div className="flex flex-col">
            {discount > 0 && <span className="text-[12px] line-through text-[var(--hz-ink-subtle)]" style={{ fontFamily: FONT_BODY }}>₹{total.toLocaleString('en-IN')}</span>}
            <span className="text-[19px] font-semibold text-[var(--hz-ink)]" style={{ fontFamily: FONT_HEAD }}>₹{finalTotal.toLocaleString('en-IN')}</span>
          </div>
          <button
            onClick={handleAdd}
            className="h-11 px-6 rounded-[10px] bg-[var(--hz-primary)] text-white text-[14px] font-semibold cursor-pointer hover:brightness-90 active:scale-[0.99] transition-all border-0"
            style={{ fontFamily: FONT_BODY, boxShadow: '0 2px 8px rgba(114,46,209,0.25)' }}
          >
        ...
        <div className="flex flex-col flex-1 min-w-0">
          ...
          <main className="flex-1 overflow-y-auto" style={{ padding: '28px 24px', scrollbarWidth: 'none' }}>
            <div className="flex flex-col gap-6" style={{ maxWidth: 1200, margin: '0 auto' }}>
              ...
              <div className="flex flex-col gap-1.5">
                <span className="text-[12px] tracking-[0.10em] uppercase text-[var(--hz-primary)] font-semibold" style={{ fontFamily: FONT_MONO }}>Painting &amp; Waterproofing</span>
