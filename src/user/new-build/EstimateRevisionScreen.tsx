...
function SegmentedControl<T extends string>({ label, options, value, onChange }: {
  ...
  return (
    <div role="radiogroup" aria-label={label} className="flex items-center gap-1 p-1 rounded-[10px] overflow-x-auto" style={{ backgroundColor: 'var(--hz-surface-muted)', scrollbarWidth: 'none' }}>
      {options.map(opt => (
        <button
          key={opt.value}
          role="radio"
          ...
          style={{
            ...
            backgroundColor: value === opt.value ? 'var(--hz-surface)' : 'transparent',
...
function AssumptionField({ title, helper, children }: { title: string; helper?: string; children: React.ReactNode }) {
  return (
    <div className="bg-[var(--hz-surface)] rounded-[14px] border border-[var(--hz-border)] p-4 sm:p-5 flex flex-col gap-3">
      <span className="text-[13px] font-semibold text-[var(--hz-ink)]" style={{ fontFamily: '"Geist Variable", sans-serif' }}>{title}</span>
      {children}
      {helper && <p className="text-[11px] text-[var(--hz-ink-subtle)] leading-[1.5] m-0" style={{ fontFamily: '"Inter Variable", sans-serif' }}>{helper}</p>}
    </div>
  )
}
  ...
  return (
    <div className="flex items-center gap-2">
      <button
        ...
        className="w-9 h-9 rounded-[9px] border border-[var(--hz-border)] flex items-center justify-center cursor-pointer bg-[var(--hz-surface)] hover:bg-[var(--hz-surface-muted)] disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
      >
        <IcoMinus />
      </button>
      <label className="relative flex-1">
        ...
        <input
          ...
          className="w-full h-9 px-3 text-center rounded-[9px] border border-[var(--hz-border)] text-[14px] font-semibold text-[var(--hz-ink)] outline-none focus:border-[var(--hz-primary)]"
          style={{ fontFamily: '"Geist Variable", sans-serif' }}
        />
        <span className="pointer-events-none absolute right-3 top-1/2 -translate-y-1/2 text-[12px] text-[var(--hz-ink-subtle)]" style={{ fontFamily: '"Sometype Mono:SemiBold", monospace' }}>SQ FT</span>
      </label>
      <button
        onClick={() => onChange(clamp(value + BUILT_UP_AREA_STEP))}
  ...
  return (
    <div className="flex flex-col gap-2">
      <div className="flex items-center justify-between">
        <span className="text-[12px] text-[var(--hz-ink-subtle)]" style={{ fontFamily: '"Sometype Mono:SemiBold", monospace' }}>{CONTINGENCY_MIN}%</span>
        <span className="text-[14px] font-semibold" style={{ fontFamily: '"Geist Variable", sans-serif', color: 'var(--hz-primary)' }}>{value}%</span>
        <span className="text-[12px] text-[var(--hz-ink-subtle)]" style={{ fontFamily: '"Sometype Mono:SemiBold", monospace' }}>{CONTINGENCY_MAX}%</span>
      </div>
      <input
        type="range"
        min={CONTINGENCY_MIN}
        ...
        aria-valuenow={value}
        style={{ accentColor: 'var(--hz-primary)', width: '100%', cursor: 'pointer' }}
      />
    </div>
  )
}
...
function CurrentEstimateCard({ version, base }: { version: number; base: RevisedEstimateResult }) {
  return (
    <div className="bg-[var(--hz-surface)] rounded-[16px] border border-[var(--hz-border)] p-5 flex flex-col gap-3">
      <div className="flex items-center justify-between">
        <span className="text-[12px] tracking-[0.10em] text-[var(--hz-ink-subtle)] uppercase" style={{ fontFamily: '"Sometype Mono:SemiBold", monospace' }}>Current Estimate</span>
        <span className="text-[12px] px-2 py-0.5 rounded-full" style={{ backgroundColor: 'var(--hz-surface-muted)', color: 'var(--hz-ink-muted)', fontFamily: '"Sometype Mono:SemiBold", monospace' }}>VERSION {version}</span>
      </div>
      ...
      <div className="flex items-center gap-4">
        <span className="text-[12px] text-[var(--hz-ink-muted)]" style={{ fontFamily: '"Inter Variable", sans-serif' }}>Average: <strong style={{ color: 'var(--hz-ink)' }}>{formatINR(base.avgTotal)}</strong></span>
        <span className="text-[12px] text-[var(--hz-ink-muted)]" style={{ fontFamily: '"Inter Variable", sans-serif' }}>Confidence: <strong style={{ color: 'var(--hz-ink)' }}>{base.confidence}%</strong></span>
      </div>
      <span className="inline-flex items-center gap-1.5 self-start text-[11px] px-2 py-1 rounded-full" style={{ backgroundColor: 'var(--hz-primary-soft)', color: 'var(--hz-primary)', fontFamily: '"Inter Variable", sans-serif' }}>
        <span className="w-1.5 h-1.5 rounded-full" style={{ backgroundColor: 'var(--hz-primary)' }} />
        Current version
      </span>
    </div>
  )
...
function RecalculatingPanel() {
  return (
    <div className="bg-[var(--hz-surface)] rounded-[16px] border border-[var(--hz-border)] p-6 flex flex-col items-center text-center gap-3">
      <div className="w-11 h-11 rounded-full flex items-center justify-center" style={{ backgroundColor: 'var(--hz-primary-soft)', animation: 'aiIconGlow 1.6s ease-in-out infinite' }}>
        <HIcon size={24} />
      </div>
      <span className="text-[12px] uppercase tracking-[0.10em]" style={{ fontFamily: '"Sometype Mono:SemiBold", monospace', color: 'var(--hz-primary)' }}>Recalculating</span>
      <p className="text-[13px] text-[var(--hz-ink-muted)] m-0" style={{ fontFamily: '"Inter Variable", sans-serif' }}>Hozie is updating materials, labour and finishing costs…</p>
    </div>
  )
}
...
return (
    <div className="flex flex-col relative" style={{ height: '100%', backgroundColor: 'var(--hz-surface)' }}>
      ...
      <div className="flex md:hidden h-14 items-center justify-between px-4 bg-[var(--hz-surface)] border-b border-[var(--hz-border)] shrink-0 z-10">
        ...
        <span className="text-[15px] font-semibold text-[var(--hz-ink)]" style={{ fontFamily: '"Geist Variable", sans-serif' }}>Estimate Revision</span>
      ...
      <div className="flex flex-1 min-h-0 relative z-10">
        ...
        <div className="flex flex-col flex-1 min-h-0">
            ...
            <div className="max-w-[1160px] mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8 flex flex-col gap-6">
              ...
              <div className="flex flex-col lg:flex-row gap-6 items-start">
                ...
                <div className="flex flex-col gap-4 w-full min-w-0" style={{ flex: '58 58 0' }}>
                  <span className="text-[12px] tracking-[0.10em] text-[var(--hz-ink-subtle)] uppercase" style={{ fontFamily: '"Sometype Mono:SemiBold", monospace' }}>Revision Controls</span>
                ...
                <div className="flex flex-col gap-4 w-full lg:shrink-0 lg:sticky lg:top-6" style={{ flex: '42 42 0', minWidth: 0 }}>
                  ...
                  {!hasChanges && (
                    <div className="bg-[var(--hz-surface)] rounded-[16px] border border-[var(--hz-border)] border-dashed p-5 text-center">
                      <p className="text-[13px] text-[var(--hz-ink-subtle)] m-0" style={{ fontFamily: '"Inter Variable", sans-serif' }}>
                  ...
                  {showRevisionResult && (
                    <>
                      ...
                      <div className="rounded-[16px] p-5 flex flex-col gap-3" style={{ backgroundColor: '#F9F5FF', border: '1px solid rgba(243,234,255,0.10)' }}>
                        ...
                        <span className="text-[11px] text-[var(--hz-ink-muted)]" style={{ fontFamily: '"Inter Variable", sans-serif' }}>Confidence: {revised.confidence}% · ₹{revised.costPerSqFt.toLocaleString('en-IN')}/sq ft</span>
                      </div>

                      {/* Before / After */}
                      <div className="bg-[var(--hz-surface)] rounded-[16px] border border-[var(--hz-border)] p-5 flex flex-col gap-3">
                        <span className="text-[12px] tracking-[0.10em] text-[var(--hz-ink-subtle)] uppercase" style={{ fontFamily: '"Sometype Mono:SemiBold", monospace' }}>Before / After</span>
                        <div className="grid grid-cols-2 gap-3">
                          <div className="flex flex-col gap-1">
                            <span className="text-[12px] uppercase tracking-[0.06em] text-[var(--hz-ink-subtle)]" style={{ fontFamily: '"Sometype Mono:SemiBold", monospace' }}>Current</span>
                            <span className="text-[14px] font-semibold text-[var(--hz-ink)]" style={{ fontFamily: '"Geist Variable", sans-serif' }}>{formatINR(baseResult.minTotal)} — {formatINR(baseResult.maxTotal)}</span>
                          </div>
                          <div className="flex flex-col gap-1">
                            <span className="text-[12px] uppercase tracking-[0.06em] text-[var(--hz-primary)]" style={{ fontFamily: '"Sometype Mono:SemiBold", monospace' }}>Revised</span>
                            <span className="text-[14px] font-semibold" style={{ fontFamily: '"Geist Variable", sans-serif', color: 'var(--hz-primary)' }}>{formatINR(revised.minTotal)} — {formatINR(revised.maxTotal)}</span>
                          </div>
                        </div>
                      ...
                      {/* Change summary */}
                      <div className="bg-[var(--hz-surface)] rounded-[16px] border border-[var(--hz-border)] p-5 flex flex-col gap-3">
                        <span className="text-[12px] tracking-[0.10em] text-[var(--hz-ink-subtle)] uppercase" style={{ fontFamily: '"Sometype Mono:SemiBold", monospace' }}>What Changed</span>
                  ...
                  {errored && (
                    <div className="flex flex-col items-center justify-center gap-2 py-8 px-5 text-center bg-[var(--hz-surface)] rounded-[16px] border border-[var(--hz-border)]">
                  ...
                  {/* Revision history */}
                  <div className="bg-[var(--hz-surface)] rounded-[16px] border border-[var(--hz-border)] p-5 flex flex-col gap-2.5">
                    <span className="text-[12px] tracking-[0.10em] text-[var(--hz-ink-subtle)] uppercase" style={{ fontFamily: '"Sometype Mono:SemiBold", monospace' }}>Estimate Version</span>
                    <div className="flex items-center justify-between">
                      <span className="text-[13px] text-[var(--hz-ink)]" style={{ fontFamily: '"Inter Variable", sans-serif' }}>Version {version}</span>
                      <span className="text-[11px] px-2 py-0.5 rounded-full" style={{ backgroundColor: 'var(--hz-primary-soft)', color: 'var(--hz-primary)', fontFamily: '"Inter Variable", sans-serif' }}>Current</span>
                    </div>
                    {hasChanges && (
                      <div className="flex items-center justify-between opacity-60">
                        <span className="text-[13px] text-[var(--hz-ink)]" style={{ fontFamily: '"Inter Variable", sans-serif' }}>Version {version + 1}</span>
                        <span className="text-[11px] px-2 py-0.5 rounded-full" style={{ backgroundColor: 'var(--hz-surface-muted)', color: 'var(--hz-ink-muted)', fontFamily: '"Inter Variable", sans-serif' }}>After applying</span>
                      </div>
                    )}
                  </div>
                </div>
          ...
          {/* Actions */}
          <div
            className="sticky bottom-0 z-20 bg-[var(--hz-surface)] border-t border-[var(--hz-border)] px-4 sm:px-6 lg:px-8 py-3.5 flex flex-col sm:flex-row items-stretch sm:items-center justify-end gap-2.5"
            style={{ boxShadow: '0 -4px 20px rgba(36,35,38,0.06)' }}
          >
            <button
              onClick={() => onNavigate('estimate-comparison')}
              className="h-11 px-5 rounded-[12px] border border-[var(--hz-border)] text-[var(--hz-ink)] text-[13px] font-medium cursor-pointer bg-transparent hover:bg-[var(--hz-surface-muted)] transition-colors order-3 sm:order-1"
              style={{ fontFamily: '"Inter Variable", sans-serif' }}
            >
              Back to comparison
            </button>
            <button
              ...
              disabled={!hasChanges}
              className="h-11 px-5 rounded-[12px] border border-[var(--hz-border)] text-[var(--hz-ink)] text-[13px] font-medium cursor-pointer bg-transparent hover:bg-[var(--hz-surface-muted)] disabled:opacity-40 disabled:cursor-not-allowed transition-colors order-2"
              style={{ fontFamily: '"Inter Variable", sans-serif' }}
            >
              Reset changes
            </button>
