...
type SortKey = 'cost' | 'quantity' | 'category'
...
function MaterialFilters({
  search, onSearch, category, onCategory, quality, onQuality, sort, onSort,
}: {
  search: string; onSearch: (v: string) => void
  category: 'all' | MaterialCategory; onCategory: (v: 'all' | MaterialCategory) => void
  quality: QualityTier; onQuality: (v: QualityTier) => void
    ...
    <div className="flex flex-col sm:flex-row sm:items-center gap-2.5 sm:gap-3">
      ...
      <div className="flex items-center gap-2 overflow-x-auto sm:overflow-visible pb-1 sm:pb-0" style={{ scrollbarWidth: 'none' }}>
        <SelectField
          ...
          value={category}
          onChange={v => onCategory(v as 'all' | MaterialCategory)}
          options={[{ value: 'all', label: 'All materials' }, ...materialCategories.map(c => ({ value: c, label: c }))]}
          minWidth={148}
        />
        <SelectField
          label="Filter by quality"
          value={quality}
          onChange={v => onQuality(v as QualityTier)}
          options={[
...
function MaterialTableRow({ mat, onOpen }: { mat: MaterialEstimate; onOpen: (m: MaterialEstimate) => void }) {
  return (
    <tr
      onClick={() => onOpen(mat)}
    ...
    >
      <td className="px-4 py-4">
        <span className="text-[14px] font-semibold text-[#242326]" style={{ fontFamily: '"Open Sans:Regular", sans-serif' }}>{mat.name}</span>
      </td>
      <td className="px-4 py-4">
        <span className="text-[13px] text-[#68636D]" style={{ fontFamily: '"Open Sans:Regular", sans-serif' }}>{mat.category}</span>
      </td>
      <td className="px-4 py-4">
        <span className="text-[13px] text-[#242326]" style={{ fontFamily: '"Open Sans:Regular", sans-serif' }}>{mat.quantity.toLocaleString('en-IN')}</span>
      </td>
      <td className="px-4 py-4">
        <span className="text-[12px] uppercase tracking-[0.06em] text-[#9A949D]" style={{ fontFamily: '"Sometype Mono:SemiBold", monospace' }}>{mat.unit}</span>
      </td>
      <td className="px-4 py-4">
        <span className="text-[13px] text-[#68636D]" style={{ fontFamily: '"Open Sans:Regular", sans-serif' }}>{formatINR(mat.rate)}</span>
      </td>
      <td className="px-4 py-4">
        <span className="text-[15px] font-semibold text-[#242326]" style={{ fontFamily: '"Google Sans Flex:SemiBold", sans-serif' }}>{formatINR(mat.estimatedCost)}</span>
      </td>
      <td className="px-4 py-4 hidden lg:table-cell">
        <ConfidenceBadge level={mat.confidence} />
      </td>
      <td className="px-4 py-4 text-right">
        <button
          onClick={e => { e.stopPropagation(); onOpen(mat) }}
          aria-label={`View details for ${mat.name}`}
...
function MaterialTable({ items, loading, onOpen }: { items: MaterialEstimate[]; loading: boolean; onOpen: (m: MaterialEstimate) => void }) {
  return (
        ...
        <table className="w-full border-collapse" style={{ minWidth: 760 }}>
          <thead>
            <tr style={{ backgroundColor: '#F4F0EC' }}>
              {['Material', 'Category', 'Est. Quantity', 'Unit', 'Rate', 'Estimated Cost', 'Confidence', 'Action'].map((h, i) => (
                <th
                  key={h}
                  scope="col"
                  className={['px-4 py-3 text-left text-[12px] sm:text-[12px] uppercase tracking-[0.08em] text-[#68636D]', i === 6 ? 'hidden lg:table-cell' : '', i === 7 ? 'text-right' : ''].join(' ')}
          ...
          <tbody>
            {loading
              ...
              : items.map(mat => <MaterialTableRow key={mat.id} mat={mat} onOpen={onOpen} />)}
          </tbody>
        </table>
      </div>
    </div>
...
function MaterialCard({ mat, onOpen }: { mat: MaterialEstimate; onOpen: (m: MaterialEstimate) => void }) {
  return (
    <div className="bg-white rounded-[14px] border border-[#E3DDD7] p-4 flex flex-col gap-3">
      <div className="flex items-start justify-between gap-3">
        <div className="flex flex-col gap-0.5 min-w-0">
          <span className="text-[14px] font-semibold text-[#242326] truncate" style={{ fontFamily: '"Open Sans:Regular", sans-serif' }}>{mat.name}</span>
          <span className="text-[12px] text-[#9A949D]" style={{ fontFamily: '"Open Sans:Regular", sans-serif' }}>{mat.category}</span>
        </div>
        <ConfidenceBadge level={mat.confidence} />
      </div>
      <div className="flex items-center justify-between gap-3">
        <div className="flex flex-col gap-0.5">
          ...
          <span className="text-[13px] text-[#242326]" style={{ fontFamily: '"Open Sans:Regular", sans-serif' }}>{mat.quantity.toLocaleString('en-IN')} {mat.unit}</span>
        </div>
        <div className="flex flex-col gap-0.5">
          ...
          <span className="text-[13px] text-[#242326]" style={{ fontFamily: '"Open Sans:Regular", sans-serif' }}>{formatINR(mat.rate)}</span>
        </div>
        <div className="flex flex-col gap-0.5 items-end">
          <span className="text-[12px] uppercase tracking-[0.08em] text-[#9A949D]" style={{ fontFamily: '"Sometype Mono:SemiBold", monospace' }}>Cost</span>
          <span className="text-[15px] font-semibold text-[#242326]" style={{ fontFamily: '"Google Sans Flex:SemiBold", sans-serif' }}>{formatINR(mat.estimatedCost)}</span>
        </div>
      </div>
      <button
...
function ErrorState({ onRetry }: { onRetry: () => void }) {
  return (
    <div className="flex flex-col items-center justify-center gap-3 py-14 px-6 text-center bg-white rounded-[16px] border border-[#E3DDD7]">
      ...
      <p className="text-[14px] text-[#68636D] m-0" style={{ fontFamily: '"Open Sans:Regular", sans-serif' }}>
        Unable to load material estimate. Try again.
      </p>
      <button
        onClick={onRetry}
...
function MaterialDetailDrawer({ mat, onClose, onAskHozie }: { mat: MaterialEstimate | null; onClose: () => void; onAskHozie: () => void }) {
  const open = mat !== null
  ...
  return (
    <>
      ...
      <div
        role="dialog"
        aria-modal="true"
        aria-label={mat ? `${mat.name} details` : 'Material details'}
        className={[
        ...
        {mat && (
          <>
            <div className="flex items-center justify-between px-5 py-4 border-b border-[#E3DDD7] shrink-0">
              <span className="text-[12px] uppercase tracking-[0.10em] text-[#9A949D]" style={{ fontFamily: '"Sometype Mono:SemiBold", monospace' }}>Material Detail</span>
              <button
                ref={closeRef}
                onClick={onClose}
            ...
            <div className="flex-1 overflow-y-auto px-5 py-5 flex flex-col gap-5">
              <div>
                <h2 className="text-[22px] font-semibold text-[#242326] m-0" style={{ fontFamily: '"Google Sans Flex:SemiBold", sans-serif' }}>{mat.name}</h2>
                <span className="text-[13px] text-[#68636D]" style={{ fontFamily: '"Open Sans:Regular", sans-serif' }}>{mat.category}</span>
              </div>

              <div className="grid grid-cols-2 gap-3">
                <div className="flex flex-col gap-1 p-3 rounded-[12px]" style={{ backgroundColor: '#FFFFFF' }}>
                  <span className="text-[12px] uppercase tracking-[0.08em] text-[#9A949D]" style={{ fontFamily: '"Sometype Mono:SemiBold", monospace' }}>Estimated Quantity</span>
                  <span className="text-[15px] font-semibold text-[#242326]" style={{ fontFamily: '"Google Sans Flex:SemiBold", sans-serif' }}>{mat.quantity.toLocaleString('en-IN')} {mat.unit}</span>
                </div>
                <div className="flex flex-col gap-1 p-3 rounded-[12px]" style={{ backgroundColor: '#FFFFFF' }}>
                  <span className="text-[12px] uppercase tracking-[0.08em] text-[#9A949D]" style={{ fontFamily: '"Sometype Mono:SemiBold", monospace' }}>Rate</span>
                  <span className="text-[15px] font-semibold text-[#242326]" style={{ fontFamily: '"Google Sans Flex:SemiBold", sans-serif' }}>{formatINR(mat.rate)} / {mat.unit}</span>
                </div>
              </div>
              ...
              <div className="flex flex-col gap-1 p-4 rounded-[12px]" style={{ backgroundColor: '#F9F5FF' }}>
                ...
                <span className="text-[26px] font-semibold" style={{ fontFamily: '"Google Sans Flex:SemiBold", sans-serif', color: '#722ED1' }}>{formatINR(mat.estimatedCost)}</span>
              </div>
              ...
              <div className="flex flex-col gap-1.5">
                ...
                <p className="text-[13px] text-[#68636D] leading-[1.6] m-0" style={{ fontFamily: '"Open Sans:Regular", sans-serif' }}>{mat.calculationBasis}</p>
              </div>
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex flex-col gap-1 items-end">
                  <span className="text-[12px] uppercase tracking-[0.08em] text-[#9A949D]" style={{ fontFamily: '"Sometype Mono:SemiBold", monospace' }}>Price updated</span>
                  <span className="text-[13px] text-[#242326]" style={{ fontFamily: '"Open Sans:Regular", sans-serif' }}>{mat.priceUpdatedAt}</span>
                </div>
              </div>

              <HozieInsightCard message={`“Actual ${mat.name.toLowerCase()} quantity will depend on structural drawings and engineer specifications.”`} />
            </div>
...
export default function MaterialEstimateScreen({
  onNavigate,
  ...
  const [selected, setSelected] = useState<MaterialEstimate | null>(null)

  // Real, requirements-driven total when this project has a real Estimate
  // Version — the static materialEstimates rows below are then scaled
  // proportionally so they still sum to that real total, rather than
  // replaced outright (the individual line items — cement, steel, etc. —
  // stay genuine material categories, just correctly sized to this
  // project). Falls back to the original hardcoded rows/total otherwise.
  const version = projectId ? getLatestEstimateVersion(projectId) : undefined
  const realMaterialCost = version?.costBreakdown.materialCost
  const staticMaterialSum = useMemo(() => materialEstimates.reduce((s, m) => s + m.estimatedCost, 0), [])
  const scaleFactor = realMaterialCost ? realMaterialCost / staticMaterialSum : 1
  const totalMaterialLabel = realMaterialCost ? formatLakh(realMaterialCost) : '₹18.2L'
  const totalMaterialRange = realMaterialCost
    ? `RANGE ${formatLakh(Math.round(realMaterialCost * 0.956))} – ${formatLakh(Math.round(realMaterialCost * 1.077))}`
    : 'RANGE ₹17.4L – ₹19.6L'

  const filtered = useMemo(() => {
    let list = materialEstimates.filter(m => m.quality === quality)
    ...
    return sorted.map(m => (scaleFactor === 1 ? m : { ...m, estimatedCost: Math.round(m.estimatedCost * scaleFactor) }))
  }, [search, category, quality, sort, scaleFactor])

  const isEmpty = status === 'ready' && filtered.length === 0
  const totalCategories = new Set(materialEstimates.map(m => m.category)).size
return (
    <div className="flex flex-col relative" style={{ height: '100%', backgroundColor: '#FFFFFF' }}>
      ...
      <div className="flex md:hidden h-14 items-center justify-between px-4 bg-white border-b border-[#E3DDD7] shrink-0 z-10">
        ...
        <span className="text-[15px] font-semibold text-[#242326]" style={{ fontFamily: '"Google Sans Flex:SemiBold", sans-serif' }}>Material Estimate</span>
      ...
      <div className="flex flex-1 min-h-0 relative z-10">
        ...
        <div className="flex flex-col flex-1 min-h-0">
            ...
            <div className="max-w-[1120px] mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8 flex flex-col gap-6">
              ...
              <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-3" style={{ animation: 'welcomeFadeUp 0.4s ease-out 0.05s both' }}>
                <div>
                  ...
                  <p className="text-[14px] sm:text-[15px] text-[#68636D] leading-[1.65] m-0 mt-2 max-w-[520px]" style={{ fontFamily: '"Open Sans:Regular", sans-serif' }}>
                    Hozie has estimated the materials, quantities and current construction costs for your project.
                  </p>
                </div>
                <div className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-full bg-[#F3EAFF] self-start shrink-0" style={{ border: '1px solid rgba(243,234,255,0.10)' }}>
                  ...
                  <span className="text-[11px] font-medium text-[#722ED1]" style={{ fontFamily: '"Open Sans:Regular", sans-serif' }}>{version?.confidence ?? 86}% AI confidence</span>
                </div>
              </div>

              {/* Summary cards */}
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4" style={{ animation: 'welcomeFadeUp 0.4s ease-out 0.1s both' }}>
                <MetricCard eyebrow="Total Material Cost" value={totalMaterialLabel} />
                <MetricCard eyebrow="Material Categories" value={String(totalCategories)} />
                <MetricCard eyebrow="Estimated Quantity" value="1,240+ units" />
                <MetricCard eyebrow="Price Basis" value="Current" sublabel="Regional estimate" />
              </div>

              {/* Hozie insight */}
              <div style={{ animation: 'welcomeFadeUp 0.4s ease-out 0.14s both' }}>
                <HozieInsightCard
                  message="Steel and cement are currently your largest material cost drivers. Final quantities should be confirmed against structural drawings."
              ...
              <div style={{ animation: 'welcomeFadeUp 0.4s ease-out 0.22s both' }}>
                {status === 'error' ? (
                  ...
                  <>
                    <MaterialTable items={filtered} loading={status === 'loading'} onOpen={setSelected} />
                    <div className="md:hidden flex flex-col gap-3">
                      {status === 'loading'
                        ? Array.from({ length: 4 }).map((_, i) => <SkeletonCard key={i} />)
                        : filtered.map(mat => <MaterialCard key={mat.id} mat={mat} onOpen={setSelected} />)}
                    </div>
                  </>
                )}
              </div>
          ...
          <EstimateFooter
            ...
            value={totalMaterialLabel}
            range={totalMaterialRange}
            primaryLabel="Continue to Labour Estimate →"
            onPrimary={() => onNavigate('labour-estimate', { project_name: projectName, location, ...(projectId ? { project_id: projectId } : {}) })}
          />
        </div>
      </div>

      <MaterialDetailDrawer mat={selected} onClose={() => setSelected(null)} onAskHozie={() => onNavigate('ai-advisor', { project_id: projectId || boqOverview.projectId })} />
    </div>
  )
}
