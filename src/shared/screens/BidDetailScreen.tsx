    ...
    <div className="min-h-full flex flex-col relative" style={{ backgroundColor: '#FFFFFF' }}>
      ...
      <header className="shrink-0 relative z-10 bg-white" style={{ borderBottom: '1px solid #F4F0EC' }}>
        <div className="flex items-center justify-between h-14 px-4 sm:px-6 lg:px-8">
          ...
          <span className="text-[11px] tracking-[0.08em] uppercase text-[#9A949D] hidden sm:block" style={{ fontFamily: FONT_MONO }}>Bid Detail</span>
        </div>
      </header>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-[1fr_320px] gap-5 items-start">
            ...
            <div className="flex flex-col gap-5 min-w-0">
              <SectionCard>
                ...
                <p className="text-[11px] tracking-[0.06em] uppercase text-[#9A949D] m-0" style={{ fontFamily: FONT_MONO }}>Your Proposed Price</p>
                <p className="text-[30px] font-semibold text-[#242326] m-0 mt-1" style={{ fontFamily: FONT_HEAD }}>{formatBidAmount(bid.amount)}</p>
                ...
                <div className="flex flex-wrap gap-6 mt-4 pt-4" style={{ borderTop: '1px solid #F4F0EC' }}>
                  <div>
                    ...
                    <p className="text-[14.5px] font-semibold text-[#242326] m-0 mt-1" style={{ fontFamily: FONT_HEAD }}>{formatBidDuration(bid.duration, bid.durationUnit)}</p>
                  </div>
                  <div>
                    <p className="text-[11px] tracking-[0.06em] uppercase text-[#9A949D] m-0" style={{ fontFamily: FONT_MONO }}>Proposed Start Date</p>
                    <p className="text-[14.5px] font-semibold text-[#242326] m-0 mt-1" style={{ fontFamily: FONT_HEAD }}>
              ...
              <SectionCard title="Contractor's Proposal">
                <p className="text-[13.5px] text-[#68636D] leading-[1.6] m-0 whitespace-pre-wrap">{bid.proposal || 'No proposal message provided.'}</p>
              </SectionCard>

              <SectionCard title="What's Included">
                <p className="text-[13.5px] text-[#68636D] leading-[1.6] m-0 whitespace-pre-wrap">{bid.included || 'No inclusions specified.'}</p>
              </SectionCard>

              <SectionCard title="Exclusions / Notes">
                <p className="text-[13.5px] text-[#68636D] leading-[1.6] m-0 whitespace-pre-wrap">{bid.exclusions || 'No exclusions or additional notes specified.'}</p>
              </SectionCard>
                ...
                <SectionCard title="Attachments">
                  <div className="flex flex-col gap-2">
                    {bid.attachments.map(doc => (
                      <div key={doc.id} className="flex items-center gap-2.5 px-3 py-2 rounded-[10px]" style={{ border: '1px solid #E3DDD7' }}>
                        {isPreviewableImage(doc.mimeType) ? <IcoImage /> : isPdf(doc.mimeType, doc.name) ? <IcoFile /> : <IcoFile />}
                        <div className="min-w-0 flex-1">
                          <p className="text-[12.5px] font-medium text-[#242326] m-0 truncate" style={{ fontFamily: FONT_BODY }}>{doc.name}</p>
            ...
            <div className="flex flex-col gap-5 min-w-0">
              <SectionCard title="Contractor">
                <div className="flex items-start gap-3">
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-1.5 flex-wrap">
                      <p className="text-[14px] font-semibold text-[#242326] m-0 truncate" style={{ fontFamily: FONT_HEAD }}>{name}</p>
                ...
                <button
                  type="button"
                  onClick={viewProfile}
                  className="mt-3 text-[12.5px] font-semibold text-[#722ED1] hover:underline cursor-pointer border-0 bg-transparent p-0"
              ...
              <SectionCard title="Project">
                <div className="flex flex-col gap-1">
                  <p className="text-[13px] font-semibold text-[#242326] m-0" style={{ fontFamily: FONT_HEAD }}>{projectName || 'Your project'}</p>
                  {propertyType && <p className="text-[12px] text-[#68636D] m-0" style={{ fontFamily: FONT_BODY }}>{propertyType}</p>}
                  {location && (
                    <span className="flex items-center gap-1.5 text-[12px] text-[#68636D] mt-0.5" style={{ fontFamily: FONT_BODY }}>
                      <IcoMapPin /> {location}
                    </span>
                  )}
                </div>
              ...
              <SectionCard title="Timeline">
                <div className="flex flex-col gap-0.5">
                  <p className="text-[12.5px] font-medium text-[#242326] m-0" style={{ fontFamily: FONT_BODY }}>Submitted</p>
                  <p className="text-[12px] text-[#68636D] m-0" style={{ fontFamily: FONT_BODY }}>{formatDate(bid.createdAt)}</p>
                </div>
              </SectionCard>
