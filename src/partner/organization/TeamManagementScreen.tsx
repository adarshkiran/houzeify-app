  ...
  return (
    <div className="rounded-[16px] bg-[var(--hz-surface)] p-5" style={{ border: '1px solid var(--hz-border)' }}>
      <div className="flex items-center gap-2.5 mb-3">
        {icon && <span className="w-8 h-8 rounded-[9px] flex items-center justify-center shrink-0" style={{ backgroundColor: 'var(--hz-primary-soft)', color: 'var(--hz-primary)' }}>{icon}</span>}
    ...
    return (
      <div className="min-h-full flex flex-col relative items-center justify-center gap-4 px-6" style={{ backgroundColor: 'var(--hz-surface)' }}>
        <div className="relative z-10 flex flex-col items-center gap-4 text-center">
          ...
          <button type="button" onClick={goToSettings} className="h-10 px-5 rounded-[12px] text-[13.5px] font-semibold border-0 cursor-pointer" style={{ backgroundColor: 'var(--hz-primary)', color: 'white', fontFamily: FONT_BODY }}>
      ...
      <main className="flex-1 overflow-y-auto relative z-10 px-4 sm:px-6 py-8">
        <div className="max-w-[720px] mx-auto flex flex-col gap-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-[11px] tracking-[0.08em] uppercase text-[var(--hz-primary)] m-0 mb-1" style={{ fontFamily: FONT_MONO }}>Team Management</p>
              <h1 className="text-[19px] font-semibold text-[var(--hz-ink)] m-0" style={{ fontFamily: FONT_HEAD }}>{companyName}</h1>
              {companyOwner && (
                <p className="text-[12px] text-[var(--hz-ink-muted)] m-0 mt-0.5" style={{ fontFamily: FONT_BODY }}>Organization Owner: {companyOwner}</p>
              )}
            </div>
          </div>
          ...
          <SectionCard title="Team Overview">
            {hasInvitedCount ? (
              <div>
                <p className="text-[26px] font-bold text-[var(--hz-ink)] m-0" style={{ fontFamily: FONT_HEAD }}>{invitedCount}</p>
                <p className="text-[12.5px] text-[var(--hz-ink-muted)] m-0 mt-1" style={{ fontFamily: FONT_BODY }}>Invitations Sent</p>
              </div>
            ) : (
              <p className="text-[13px] text-[var(--hz-ink-subtle)] m-0" style={{ fontFamily: FONT_BODY }}>No team invitations have been recorded in this session.</p>
            )}
          </SectionCard>
          ...
          <SectionCard icon={<IcoTeam />} title="Confirmed Members">
            {membersStatus === 'loading' && (
              <p className="text-[13px] text-[var(--hz-ink-subtle)] m-0" style={{ fontFamily: FONT_BODY }}>Loading members…</p>
            )}
            {membersStatus === 'error' && (
              <p className="text-[13px] text-[var(--hz-ink-subtle)] m-0" style={{ fontFamily: FONT_BODY }}>Unable to load your team's members right now.</p>
            )}
            {membersStatus === 'loaded' && members && (
              <div className="flex flex-col gap-2">
                {members.map(member => {
                  ...
                  return (
                    <div key={member.id} className="flex flex-col gap-2 rounded-[10px] px-3.5 py-2.5" style={{ backgroundColor: 'var(--hz-surface)', border: '1px solid var(--hz-border)' }}>
                      <div className="flex items-center justify-between gap-3 flex-wrap">
                        <span className="text-[13px] font-medium text-[var(--hz-ink)] truncate" style={{ fontFamily: FONT_BODY }}>
                        ...
                        <span className="flex items-center gap-2 shrink-0 flex-wrap">
                          {canEditThisMember ? (
                            ...
                            <span className="px-2 py-0.5 rounded-full text-[11px] font-semibold" style={{ backgroundColor: 'var(--hz-primary-soft)', color: 'var(--hz-primary)', fontFamily: FONT_MONO }}>
                              {ROLE_LABELS[member.role as MemberRole] ?? member.role}
                            </span>
                          )}
                          <span className="text-[11px] font-semibold" style={{ color: 'var(--hz-ink-muted)', fontFamily: FONT_MONO }}>{member.status}</span>
                        </span>
                      </div>
                      ...
                      {canEditThisMember && (
                        <div className="flex items-center gap-3 flex-wrap">
                          {member.status === 'active' && (
                            <button type="button" disabled={busy} onClick={() => changeMemberStatus(member.id, 'suspended')} className="min-h-[44px] text-[12px] font-semibold cursor-pointer border-0 bg-transparent p-0" style={{ color: 'var(--hz-ink-muted)', fontFamily: FONT_BODY }}>
                              Suspend
                            </button>
                          )}
                          {member.status === 'suspended' && (
                            <button type="button" disabled={busy} onClick={() => changeMemberStatus(member.id, 'active')} className="min-h-[44px] text-[12px] font-semibold cursor-pointer border-0 bg-transparent p-0" style={{ color: 'var(--hz-primary)', fontFamily: FONT_BODY }}>
                              Reactivate
                            </button>
                          )}
                          {confirmRemoveId === member.id ? (
                            <span className="flex items-center gap-3">
                              <span className="text-[12px]" style={{ color: 'var(--hz-ink-muted)', fontFamily: FONT_BODY }}>Remove this member?</span>
                              <button type="button" disabled={busy} onClick={() => confirmRemoveMember(member.id)} className="min-h-[44px] text-[12px] font-semibold cursor-pointer border-0 bg-transparent p-0" style={{ color: 'var(--hz-danger)', fontFamily: FONT_BODY }}>
                                {busy ? 'Removing…' : 'Confirm remove'}
                              </button>
                              <button type="button" disabled={busy} onClick={() => setConfirmRemoveId(null)} className="min-h-[44px] text-[12px] font-semibold cursor-pointer border-0 bg-transparent p-0" style={{ color: 'var(--hz-ink-muted)', fontFamily: FONT_BODY }}>
                                Cancel
                              </button>
                            </span>
                          ) : (
                            <button type="button" disabled={busy} onClick={() => setConfirmRemoveId(member.id)} className="min-h-[44px] text-[12px] font-semibold cursor-pointer border-0 bg-transparent p-0" style={{ color: 'var(--hz-danger)', fontFamily: FONT_BODY }}>
                              Remove
                            </button>
                          )}
                        </div>
                      ...
                      {memberActionError?.id === member.id && (
                        <p role="alert" className="text-[12px] m-0" style={{ color: 'var(--hz-danger)', fontFamily: FONT_BODY }}>{memberActionError.message}</p>
                      )}
                    </div>
                  )
                })}
              ...
              <div className="mt-4 pt-4" style={{ borderTop: '1px solid var(--hz-surface-muted)' }}>
                {addOpen ? (
                  <div className="flex flex-col gap-3">
                    <div>
                      <label htmlFor="add-member-email" className="block text-[12px] font-semibold text-[var(--hz-ink)] mb-1" style={{ fontFamily: FONT_BODY }}>Email</label>
                      <input
                        id="add-member-email"
                        type="email"
                        value={addEmail}
                    ...
                    <div>
                      <label htmlFor="add-member-role" className="block text-[12px] font-semibold text-[var(--hz-ink)] mb-1" style={{ fontFamily: FONT_BODY }}>Role</label>
                      <select
                        id="add-member-role"
                        value={addRole}
                    ...
                    {addError && <p role="alert" className="text-[12px] m-0" style={{ color: 'var(--hz-danger)', fontFamily: FONT_BODY }}>{addError}</p>}
                    <div className="flex items-center gap-4">
                      <button type="button" disabled={addBusy || !addEmail.trim()} onClick={submitAddMember} className="h-11 px-5 rounded-[12px] text-[13.5px] font-semibold border-0 cursor-pointer" style={{ backgroundColor: 'var(--hz-primary)', color: 'white', fontFamily: FONT_BODY, opacity: addBusy ? 0.6 : 1 }}>
                        {addBusy ? 'Adding…' : 'Add member'}
                      </button>
                      <button type="button" disabled={addBusy} onClick={() => { setAddOpen(false); setAddError(null) }} className="min-h-[44px] text-[13px] font-medium cursor-pointer border-0 bg-transparent p-0" style={{ color: 'var(--hz-ink-muted)', fontFamily: FONT_BODY }}>
                        Cancel
                      </button>
                    </div>
                  </div>
            ...
            {emails.length > 0 ? (
              <>
                <div className="flex flex-col gap-2 mb-4">
                  {emails.map(email => (
                    <div key={email} className="flex items-center justify-between gap-3 rounded-[10px] px-3.5 py-2.5" style={{ backgroundColor: 'var(--hz-surface)', border: '1px solid var(--hz-border)' }}>
                      <span className="text-[13px] font-medium text-[var(--hz-ink)] truncate" style={{ fontFamily: FONT_BODY }}>{email}</span>
                      <span className="text-[11px] font-semibold shrink-0" style={{ color: 'var(--hz-ink-muted)', fontFamily: FONT_MONO }}>Invitation sent</span>
                    </div>
                  ))}
                </div>
                <p className="text-[11.5px] text-[var(--hz-ink-subtle)] m-0 mb-3" style={{ fontFamily: FONT_BODY }}>
              ...
              <div className="flex flex-col items-start gap-2">
                <p className="text-[14px] font-semibold text-[var(--hz-ink)] m-0" style={{ fontFamily: FONT_HEAD }}>No team invitations yet.</p>
                <p className="text-[13px] text-[var(--hz-ink-subtle)] m-0" style={{ fontFamily: FONT_BODY }}>Invite people through Team Setup to begin building your team.</p>
          ...
          {hasTeamSummary && (
            <SectionCard icon={<IcoTeam />} title="Team Summary">
              <p className="text-[13.5px] font-semibold text-[var(--hz-ink)] m-0" style={{ fontFamily: FONT_HEAD }}>{teamSummary}</p>
              <p className="text-[11.5px] text-[var(--hz-ink-subtle)] m-0 mt-1.5" style={{ fontFamily: FONT_BODY }}>Summary from the most recent Team Setup invitation flow.</p>
            </SectionCard>
          )}

          {/* Team Roles — reference only */}
          <SectionCard icon={<IcoShield />} title="Team Roles">
            <p className="text-[12px] text-[var(--hz-ink-subtle)] m-0 mb-3" style={{ fontFamily: FONT_BODY }}>
          ...
          <div className="rounded-[16px] p-5 flex items-center justify-between gap-3 flex-wrap" style={{ border: '1px dashed var(--hz-border)', backgroundColor: 'var(--hz-surface)' }}>
            <div>
              <p className="text-[13px] font-semibold text-[var(--hz-ink)] m-0" style={{ fontFamily: FONT_HEAD }}>Team Member Details</p>
              <p className="text-[12px] text-[var(--hz-ink-subtle)] m-0 mt-0.5" style={{ fontFamily: FONT_BODY }}>Available once invited people become confirmed members.</p>
            </div>
            <span className="h-9 px-3.5 rounded-[10px] text-[12.5px] font-semibold flex items-center" style={{ backgroundColor: 'var(--hz-surface-muted)', color: 'var(--hz-ink-subtle)', fontFamily: FONT_BODY }}>
              Coming soon
            </span>
          </div>
        </div>
